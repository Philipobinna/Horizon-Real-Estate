...
import { fullName } from "../../lib/middleware/schema";
...
export class Controller {
  ...
  static async deleteUser(req: Request, res: Response) {
    try {
      ...
      const deleteUser = await AUTH.findOne({ _id: id });
      if (!deleteUser) {
  ...
  static async getUsers(req: Request, res: Response) {
    try {
      const getAllUsers = await AUTH.find({ role: "user"});
      return res.status(200).json({
        message: "You have all your customers",
        getAllUsers,
      });
    } catch (error) {
      res.status(500).json(error);
